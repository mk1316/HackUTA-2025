...
import { convertSyllabusToEvents } from '../utils/syllabusConverter';
...
interface SyllabusResultsProps {
  readonly result: SyllabusData;
  readonly onGenerateHumorousSummary: () => void;
}
...
const getEventIcon = (type: string) => {
  ...
  } else if (lowerType.includes('office') || lowerType.includes('office_hours')) {
    return (
  ...
  } else if (lowerType.includes('class') || lowerType.includes('lecture')) {
    return (
...
const getEventTypeDot = (type: string) => {
  ...
  } else if (lowerType.includes('office')) {
...
export default function SyllabusResults({ result, onGenerateHumorousSummary }: SyllabusResultsProps) {
  ...
  const allEvents = [
    ...(result.homework?.map(item => ({ 
      ...item, 
      type: 'Assignment',
      time: item.due_time // Add time field for display
    })) || []),
    ...(result.exams?.map(item => ({ 
      ...item, 
      type: item.type || 'Exam',
      due_date: item.date, // Map date to due_date for consistent display
    })) || []),
    ...
    ...(result.office_hours?.map(item => ({ 
      title: `Office Hours - ${item.day}`,
      due_date: item.start_date,
      time: item.time,
      description: `${item.time}${item.location ? ' - ' + item.location : ''}`,
      type: 'Office Hours'
    })) || []),
    ...(result.class_meetings?.map(item => ({ 
      title: `Class - ${item.days.join(', ')}`,
      due_date: item.start_date,
      time: item.time,
      description: `${item.time}${item.location ? ' - ' + item.location : ''}`,
      type: 'Class'
  ...
  const handleContinueToCalendar = () => {
    // Convert syllabus data to events
    const events = convertSyllabusToEvents(result);
    
    // Store in sessionStorage
    const calendarData = {
      events,
      courseName: result.course_name || result.course_code || 'Course',
    };
    sessionStorage.setItem('syllabusCalendarData', JSON.stringify(calendarData));
        ...
        {allEvents.length > 0 ? (
          <div className="space-y-0">
            {allEvents.map((event, index) => (
              <div key={`event-${index}-${'title' in event ? event.title : event.type}`} className="flex items-center p-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                {/* Event Icon */}
                  ...
                  <div className="flex items-center space-x-4 mt-1 text-xs text-gray-500 dark:text-gray-400">
                    <div className="flex items-center flex-wrap gap-2">
                      <div className="flex items-center">
                        ...
                        {event.type === 'Office Hours' || event.type === 'Class' ? (
                          <span className="text-green-600 dark:text-green-400 font-medium">
                            🔁 Recurring Weekly
                          </span>
                        ) : (
                          formatDate((event as any).due_date || (event as any).date || '')
                        )}
                      </div>
                      {(event as any).time && (
